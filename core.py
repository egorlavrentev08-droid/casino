...
from config import DAILY_BONUS, MAX_BALANCE_FOR_BONUS
...
def register_core(dp):
    ...
    async def cmd_pay(message: types.Message):
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"pay_confirm_{sender_id}_{receiver_id}_{amount}"),
    ...
    async def pay_confirm(callback: types.CallbackQuery):
        ...
        # Проверяем, что подтверждает именно ОТПРАВИТЕЛЬ
