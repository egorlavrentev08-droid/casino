...
# Хранилище сообщений аукциона (для редактирования)
auction_messages = {}
...
def register_gold(dp):
    ...
    async def gold_auc(message: types.Message, page: int = 1, edit_msg: types.Message = None):
        ...
        if edit_msg:
            ...
            msg = await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard), parse_mode="Markdown")
            auction_messages[user_id] = msg
