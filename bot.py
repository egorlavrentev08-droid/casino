...
from backups import backup_manager
...
async def main():
    ...
    # Восстанавливаем БД из последнего бэкапа (если есть)
    ...
    asyncio.create_task(backup_manager.start_backup_loop())
    
    # Запуск поллинга
    logger.info("🎰 Бот запущен и готов к работе!")
    print("🎰 КАЗИНО БОТ ЗАПУЩЕН!")
    print(f"📊 Бот @{(await bot.get_me()).username}")
    print(f"📁 Папка бэкапов: {backup_manager.backup_dir}")
