...
def register_admin(dp):
    ...
    async def cmd_log(message: types.Message):
        ...
        try:
            ...
            text = f"📋 **Логи пользователя {target}** (последние 20):\n\n"
            for log in logs:
                text += f"🕐 {log['timestamp'][:19]}\n"
                text += f"📌 {log['type']}"
                if log['amount'] != 0:
                    text += f" | {log['amount']:.2f}"
                    if 'руб' in log['type'] or log['type'] in ['admin_give', 'admin_take', 'transfer_out', 'transfer_in', 'roulette_bet', 'roulette_win', 'mines_bet', 'mines_win', 'mines_lose', 'joker_bet', 'joker_win', 'joker_lose', 'daily_bonus']:
                        text += "₽"
                    elif 'gold' in log['type']:
                        text += " г"
                if log['fee'] > 0:
