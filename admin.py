...
def register_admin(dp):
    ...
    # ===== БЭКАП В TELEGRAM (СОХРАНИТЬ) =====
    @dp.message(Command("save_db"))
    async def cmd_save_db(message: types.Message):
        """Сохранить бэкап БД в Telegram"""
        if message.from_user.id != ADMIN_ID:
            await message.answer("❌ Доступ запрещён")
            return
        ...
        if not os.path.exists("casino_bot.db"):
        ...
        backup_name = f"casino_bot_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
        
        await message.bot.send_document(
            chat_id=ADMIN_ID,
            document=types.BufferedInputFile(file_data, filename=backup_name),
            caption=f"💾 Бэкап от {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}"
        )
        
        await message.answer(f"✅ Бэкап сохранён в Telegram как `{backup_name}`", parse_mode="Markdown")
        
        log_action(ADMIN_ID, "admin", "save_db_backup")
    
    # ===== ВОССТАНОВЛЕНИЕ ИЗ TELEGRAM =====
    @dp.message(Command("restore_db"))
    async def cmd_restore_db(message: types.Message):
        """Восстановить БД из присланного файла (ответом на файл)"""
        if message.from_user.id != ADMIN_ID:
            await message.answer("❌ Доступ запрещён")
            return
        ...
        if not message.reply_to_message or not message.reply_to_message.document:
            await message.answer(
                "❌ **Как восстановить:**\n\n"
                "1. Найди сообщение с бэкапом\n"
                "2. Ответь на него командой `/restore_db`\n\n"
                "💡 Пример: `/restore_db` (ответом на файл)",
                parse_mode="Markdown"
            )
            return
        
        await message.answer("🔄 Восстанавливаю базу данных... Подожди секунду!")
        
        # Скачиваем файл
        file = await message.bot.get_file(message.reply_to_message.document.file_id)
        await file.download_to_drive("casino_bot.db")
        ...
        try:
            ...
            if result[0] != "ok":
                await message.answer("❌ Файл повреждён! Восстановление отменено.")
                return
        except Exception as e:
            await message.answer(f"❌ Ошибка: {e}")
            return
        
        log_action(ADMIN_ID, "admin", "restore_db_from_backup")
    ...
    # ===== ВОССТАНОВИТЬ ИЗ ПОСЛЕДНЕГО БЭКАПА В ПАПКЕ =====
    ...
    async def cmd_backups(message: types.Message):
        ...
        text += "\n💡 `/restore` — восстановить последний\n💡 `/backup` — создать новый\n💡 `/save_db` — сохранить в Telegram\n💡 `/restore_db` — восстановить из Telegram"
        await message.answer(text, parse_mode="Markdown")
