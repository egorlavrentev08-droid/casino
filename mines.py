...
def register_mines(dp):
        ...
        try:
            ...
            if len(parts) < 2:
                await message.answer(
                    ...
                    "• Можно забрать выигрыш в любой момент\n\n"
                    "📈 **Множители:**\n"
                    "1⭐ → x1.22  | 6⭐ → x4.45  | 11⭐ → x25.79 | 16⭐ → x481.70\n"
                    "2⭐ → x1.54  | 7⭐ → x6.04  | 12⭐ → x40.33 | 17⭐ → x1284.60\n"
                    "3⭐ → x1.96  | 8⭐ → x8.36  | 13⭐ → x66.20 | 18⭐ → x4625.40\n"
                    "4⭐ → x2.54  | 9⭐ → x11.83 | 14⭐ → x115.16 | 19⭐ → x18502.00\n"
                    "5⭐ → x3.34  | 10⭐ → x17.19 | 15⭐ → x219.00 | 20⭐ → x115620.00\n\n"
                    f"📊 RTP: {MINES_RTP*100:.0f}%",
                    parse_mode="Markdown"
                )
                return
    ...
    async def mines_callback(callback: types.CallbackQuery):
        ...
        # ===== ОСНОВНАЯ ЗАЩИТА: проверка принадлежности игры =====
        game = mines_games.get(user_id)
        
        if not game:
            await callback.answer(
                "❌ У тебя нет активной игры!\n\nНапиши /mines 100 чтобы начать свою игру.", 
                show_alert=True
            )
            # Пытаемся удалить чужое сообщение с кнопками
            try:
        ...
        if now - game.last_update < 0.5:
            await callback.answer("⏳ Не так быстро! Подожди секунду.", show_alert=False)
            return
        ...
        if action[1] == 'cashout':
            ...
            if win > 0:
                ...
                await callback.message.edit_text(
                    ...
                    f"⭐ Открыто клеток: {game.opened_count}/20\n"
