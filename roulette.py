...
async def roulette_game(message: types.Message, bet: float, bet_type, bet_value):
    ...
    try:
        ...
        await message.answer(
            ...
            f"📁 Файл `roleta.gif.mp4` должен быть в корневой папке",
            parse_mode="Markdown"
        )
        # Возвращаем ставку
        ...
        await message.answer(
            f"❌ **Ошибка при отправке гифки:** {str(e)}\n"
            f"💰 Ставка возвращена.",
            parse_mode="Markdown"
        )
        update_balance(user_id, bet)
    ...
    if is_win:
        ...
        # Ставка уже списана до анимации
        result_text = (
            f"❌ **ПРОИГРЫШ** ❌\n\n"
            f"{color_emoji} **Выпало:** {win_num} ({win_color.upper()})\n"
            f"🎲 Ставка на: {bet_desc}\n"
        ...
        try:
            ...
            if len(parts) < 3:
                await message.answer(
                    ...
                    f"🎲 RTP рулетки: {ROULETTE_RTP*100:.0f}%",
                    parse_mode="Markdown"
                )
                return
