...
class JokerGame:
    ...
    def reveal(self, row_idx, col_idx):
        ...
        Открывает карту
        Возвращает:
        - 'win' + множитель, если открыта безопасная карта
        ...
        if is_safe:
            ...
            # +30% за текущий раунд, +5% за каждый предыдущий
            multiplier = 1.0
            for i in range(self.won_rounds):
                multiplier += 0.30 + (i * 0.05)
    ...
    def get_total_win(self):
        ...
        for i in range(self.won_rounds):
            total_multiplier += 0.30 + (i * 0.05)
        
        return self.bet * total_multiplier
...
def register_joker(dp):
    ...
    async def joker_callback(callback: types.CallbackQuery):
        ...
        if res[0] == 'win':
            ...
            total_mult = current_win / game.bet if game.bet > 0 else 1
            ...
            await callback.message.edit_text(
                ...
                f"🎉 **Раунд {game.won_rounds} выигран!** +{round_mult:.1f}x\n\n"
                f"❓ Продолжай открывать безопасные карты или забери выигрыш!",
                reply_markup=game.get_current_board(),
                parse_mode="Markdown"
            )
            
            await callback.answer(f"✅ Победа в раунде {game.won_rounds}! +{round_mult:.1f}x")
            return
