...
class JokerGame:
    ...
    def cancel(self):
        ...
        update_balance(self.uid, self.bet)
        add_transaction(self.uid, "joker_cancel", self.bet, 0)
        self.active = False
        return self.bet
    ...
    def get_current_board(self):
        ...
        kb.row(
            InlineKeyboardButton(text="💰 ЗАБРАТЬ", callback_data="joker_cashout"),
            InlineKeyboardButton(text="❌ ОТМЕНА", callback_data="joker_cancel")
        )
        return kb.as_markup()
...
def register_joker(dp):
    ...
    async def cmd_joker(message: types.Message):
        ...
        try:
            ...
            # Создаём игру
            game = JokerGame(user_id, bet, username)
            joker_games[user_id] = game
            
            await message.answer(
                f"🃏 **ДЖОКЕР** | 👤 @{username or f'id{user_id}'}\n"
                f"🎲 Ставка: {bet:.2f}₽\n"
                f"📈 Множитель: x1.0\n\n"
                f"✅ 2 безопасные карты | ❌ 1 джокер\n"
    ...
    async def joker_callback(callback: types.CallbackQuery):
        ...
        if not game:
            await callback.answer("❌ У тебя нет активной игры! Используй /joker", show_alert=True)
            return
        ...
        if callback.data == "joker_cashout":
            ...
            if win > 0:
                ...
                await callback.message.edit_text(
                    ...
                    f"🎲 {game.bet:.2f}₽ → x{total_mult:.3f} | ✅ {game.won_rounds} раундов",
                    parse_mode="Markdown"
                )
            else:
                await callback.message.edit_text(
                    f"❌ **@{game.owner_username} не выиграл ни одного раунда!**\n"
        ...
        if res[0] == 'lose':
            ...
            await callback.message.edit_text(
                f"💀 **@{game.owner_username} открыл джокера!**\n"
                f"🎲 {game.bet:.2f}₽ проиграно | ✅ {game.won_rounds} раундов\n\n"
        ...
        if res[0] == 'win':
            ...
            await callback.message.edit_text(
                f"🃏 **ДЖОКЕР** | 👤 @{game.owner_username}\n"
                f"🎲 {game.bet:.2f}₽ | ✅ {game.won_rounds} раундов\n"
                f"📈 x{total_mult:.3f} | 💰 {current_win:.2f}₽\n\n"
                f"🎉 +{round_mult:.2f}x за раунд!",
                reply_markup=game.get_current_board(),
                parse_mode="Markdown"
            )
